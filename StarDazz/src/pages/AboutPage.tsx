  ...
  return (
    <div className="mx-auto w-full max-w-[min(100%,96rem)] px-4 py-10 sm:px-6 sm:py-14">
      <nav className="text-sm text-zinc-500">
        ...
        <span className="text-zinc-400">{t('about.crumbCurrent')}</span>
      </nav>

      <header className="mt-8 max-w-2xl">
        <h1 className="text-3xl font-medium tracking-tight text-zinc-50 sm:text-4xl">
          {t('about.title')}
        </h1>
        <p className="mt-3 text-zinc-500">{t('about.subtitle')}</p>
      </header>
        ...
        <div className="flex min-h-full w-full flex-col rounded-2xl border border-white/[0.08] bg-zinc-950/60">
          ...
          <div className="min-h-0 flex-1 overflow-auto px-4 py-4 lg:min-h-[min(70vh,36rem)]">
            {value.trim() ? (
              ...
              <p className="text-sm text-zinc-600">{t('about.emptyPreview')}</p>
            )}
          </div>
        </div>
      </div>
